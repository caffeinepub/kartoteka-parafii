...
import {
  useDeleteStatisticEntry,
  useGetAllParishioners,
  useGetPaginatedStatisticEntries,
  useUpdateStatisticEntry,
} from "../hooks/useQueries";
import { generateParishPDF } from "../lib/pdfGenerator";

export default function Statystyki() {
  const { data: statisticsData, isLoading } = useGetPaginatedStatisticEntries(
    1,
    1000,
  );
  ...
  const [editingStatistic, setEditingStatistic] = useState<{
    id: bigint;
    data: StatisticEntry;
  } | null>(null);
  ...
  const sacramentCounts = {
    ...
    funeral:
      parishioners.filter((p) => p.sacraments.funeralYear).length +
      parishioners.reduce(
        (sum, p) =>
          sum + p.family.filter((f) => f.sacraments.funeralYear).length,
        0,
      ),
  };
  ...
  const exportStatisticsPDF = () => {
    ...
    toast.success(
      "Raport statystyczny został wygenerowany - okno drukowania otworzy się automatycznie",
    );
  };
                  ...
                  >
                    <div>
                      <p className="text-sm font-medium">
                        {new Date(
                          Number(stat.timestamp) / 1000000,
                        ).toLocaleDateString("pl-PL")}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Frekwencja: {Number(stat.sundayAttendance)} | Komunia:{" "}
                        {Number(stat.communionCount)}
                      </p>
                    </div>
                  </div>
                ))}
